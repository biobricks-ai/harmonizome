## script provided by Harmonizome at https://maayanlab.cloud/Harmonizome/static/harmonizomedownloader.py
...
def download_datasets(selected_datasets, selected_downloads, decompress=False):
    for dataset, path in selected_datasets:
        if not os.path.exists("raw/" + dataset):
            os.mkdir("raw/" + dataset)

        for downloadable in selected_downloads:
            url = 'https://maayanlab.cloud/static/hdfs/harmonizome/data/%s/%s' %\
                  (path, downloadable)
            response = requests.get(url, stream=True)
            # normalized_name = str(dataset).replace(" ", "_")  # get rid of spaces in path names
            filename = 'raw/%s/%s' % (dataset, downloadable)
